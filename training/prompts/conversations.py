    ...
    def get_separate_prompt_with_to_loss(self):
        ...
        if self.sep_style == SeparatorStyle.LLAMA2:
            ...
            for i, (role, message, to_loss) in enumerate(self.messages):
                if to_loss:
                    if message:
                        separate_prompts.append((ret + role, False))
                        separate_prompts.append((message + seps[i%2], True))
                        ret = ""
                    else:
                        separate_prompts.append((ret + role, False))
                else:
                    if message:
                        separate_prompts.append((ret + role + message + seps[i%2], False))
                    else:
                        separate_prompts.append((ret + role, False))
                    ret = ""
            ...
            for i, (role, message, to_loss) in enumerate(self.messages):
                if to_loss:
                    if message:
                        # separate_prompts.append((ret + role + " ", False))
                        # We do not add space here because a space in tail will be tokenized and
                        # the model needs to conditioned on this space
                        separate_prompts.append((ret + role, False))
                        separate_prompts.append((message + seps[i%2], True))
                        ret = ""
                    else:
            ...
            for i, (role, message, to_loss) in enumerate(self.messages):
                if to_loss:
                    if message:
                        ...
                        separate_prompts.append((ret, False))
                        ret = f"{message.strip()}<|eot_id|>"
                        separate_prompts.append((ret, True))
                        ret = ""
                    else:
                        ret += f"<|start_header_id|>{role}<|end_header_id|>\n\n"
                        separate_prompts.append((ret, False))
            ...
            ret = "" if system_prompt == "" else system_prompt + self.sep + "\n"
            for role, message, to_loss in self.messages:
                if to_loss:
                    if message:
                        ret += role + "\n"
                        separate_prompts.append((ret, False))
                        separate_prompts.append((message + self.sep, True))
                        ret = "\n"
                    else:
                        ret += role + "\n"
                        separate_prompts.append((ret, False))
...
register_conv_template(
    Conversation(
        name="k2",
        system_template="<|sys_start|> {system_message} <|sys_end|>",
        roles=("<|user_start|>", "<|user_end|>"),
...
if __name__ == "__main__":
    ...
    # conv.append_message(conv.roles[0], "How was it going?")
    # conv.append_message(conv.roles[1], "It was going well.", True)
    # conv.append_message(conv.roles[0], "What are you doing?")
    # conv.append_message(conv.roles[1], "I'm doing my homework.", True)
    ...
    # inputs = tokenize_conversation(conv, tokenizer)
    # print(inputs)
    # print(conv.hightlight_with_to_loss())
    # print(conv.get_separate_prompt_with_to_loss())
    # conv.append_message(conv.roles[1], None)
    # all_prompt = ""
    # for (prompt, to_loss) in conv.get_separate_prompt_with_to_loss():
    #     all_prompt += prompt
    #     print(prompt, to_loss)
    # print(all_prompt)
    # print("\n\n")
    # print(conv.hightlight_with_to_loss())
    # print("\n\n")
    # print(conv.get_prompt())
    # print(all_prompt == conv.get_prompt())
    ...
    conv.set_system_message("You are a pirate chatbot who always responds in pirate speak!")
    conv.append_message(conv.roles[0], "Hello!")
    conv.append_message(conv.roles[1], "Hi!", True)
    conv.append_message(conv.roles[0], " How are you?")
    conv.append_message(conv.roles[1], "I'm fine, thanks! ", True)
