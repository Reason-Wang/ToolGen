...
from utils.utils import get_toolbench_name, AllowKeyWordsProcessor, DisjunctiveTrie
import pandas as pd
import os
...
def process_retrieval_ducoment(documents_df):
    ...
    for row in documents_df.itertuples():
        ...
        toolbench_name_to_doc_id[toolbench_name] = row.docid
    return ir_corpus, toolbench_name_to_doc_id
    
# data_path = f"data/retrieval/{stage}"
data_path = "data/retrieval/corpus_G123.tsv"
documents_df = pd.read_csv(data_path, sep='\t')
ir_corpus, toolbench_name_to_doc_id = process_retrieval_ducoment(documents_df)



with open("../datasets/toolgen/G3_instruction_queries.json", "r") as f:
    queries = json.load(f)
ir_test_queries = {int(query['id']): query['query'] for query in queries}
...
query_file = "data/test_instruction/G3_instruction.json"
with open(query_file, "r") as f:
    queries = json.load(f)
ir_relevant_docs = {}
for query in queries:
    query_id = query['query_id']
    for tool in query['relevant APIs']:
        tool_name = tool[0]
        api_name = tool[1]
        toolbench_name = get_toolbench_name(tool_name, api_name)
        ir_relevant_docs.setdefault(query_id, set()).add(toolbench_name_to_doc_id[toolbench_name])
...
indexing = "Atomic"
model_name_or_path = "reasonwang/VAgent-Llama-3-8B-Planning-Full"
constrain = True
